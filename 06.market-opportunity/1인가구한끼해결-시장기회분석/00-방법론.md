...
| DOS(§4~5, 시장가중 확장) | **완료(재구성됨)** — [03-DOS매트릭스.md](./03-DOS매트릭스.md). MR을 Pain별이 아닌 **인물별**로 부여하고, **SAM 기준(확장성)·SOM 기준(1년차 실행)** 두 버전을 병기하는 방식으로 전면 재작성 |
| AOS×DOS 혼합 매트릭스(§6 토론의 시각화) | **완료(갱신됨)** — [04-AOSxDOS-혼합매트릭스.md](./04-AOSxDOS-혼합매트릭스.md). SAM 기준·SOM 기준 두 장의 `quadrantChart`로 재구성, X=AOS·Y=DOS |
