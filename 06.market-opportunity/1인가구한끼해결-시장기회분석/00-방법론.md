...
> 📎 참고(외부 Notion, 이 리포지토리에서는 접근 확인 안 됨): "매트릭스에서 사분면 상하 좌우를 가르는 수치 기준점은 어디인가?" — AOS를 실제로 계산할 때(§3) 우리 프로젝트 기준으로 이 질문에 답해야 한다.
...
이 방법론을 세팅한 상태로 **작업 대기**한다. 다음 지시로 §3의 5단계(① Pain 리스트 정리 → ⑤ Matrix 시각화)를 실제로 수행하되, §2의 두 질문(어느 단계의 Pain List를 쓸지, 어느 스펙트럼 유형을 쓸지)에 먼저 답해야 한다 — 후보 자료:

| 후보 자료 | 위치 |
| --- | --- |
| Market Segment 직접 매핑 4종 Pain | [01-페르소나.md](../../05.persona-journey-map/1인가구한끼해결-페르소나여정/01-페르소나.md) |
| 스펙트럼 12종 Pain(①1차 생성, 미검증) | [02-페르소나스펙트럼.md](../../05.persona-journey-map/1인가구한끼해결-페르소나여정/02-페르소나스펙트럼.md) |
| 7단계 CJM 여정별 Pain Point + 개선기회 | [06-고객여정지도-종합판.md](../../05.persona-journey-map/1인가구한끼해결-페르소나여정/06-고객여정지도-종합판.md) |
