...
### 🖼️ 한눈에 보기 — 포터의 가치사슬 다이어그램

```mermaid
graph TB
    subgraph SUPPORT["🏛️ 지원활동 (Support Activities)"]
        direction LR
        FI["🏢 기업 인프라"]
        HR["👥 인적자원 관리"]
        TD["🧪 기술 개발"]
    ...
    subgraph PRIMARY["⚙️ 주요활동 (Primary Activities)"]
        direction LR
        IL["🚚 투입·조달<br/>물류"] --> OP["🏭 운영·생산"] --> OL["📤 산출·배송<br/>물류"] --> MS["📣 마케팅<br/>및 영업"] --> SV["🛠️ 서비스"]
    end

    SUPPORT --> PRIMARY
    PRIMARY --> MARGIN(["💰 마진 (Margin)"])

    classDef support fill:#ede9fe,stroke:#6d28d9,color:#4c1d95;
    classDef primary fill:#e0f2fe,stroke:#0369a1,color:#0c4a6e;
    classDef margin fill:#fef3c7,stroke:#b45309,color:#78350f;
    class FI,HR,TD,PR support;
    class IL,OP,OL,MS,SV primary;
    class MARGIN margin;
```

> 지원활동(위)은 주요활동(아래) 전체를 가로질러 지탱하는 인프라이고, 주요활동은 투입→운영→산출→마케팅→서비스의 순서로 흘러 최종적으로 마진을 만들어낸다. 실제 분석에서는 이 9개 박스 중 **원가·차별화가 큰 2~3개를 색으로 강조**해 "이 산업은 어디에 힘을 줬는지"를 표시하면 좋다(개별 분석 파일 참고).
...
## 4. 출력 포맷 지시문 (재현성을 위한 산출물 구조)

이 방법론으로 분석을 수행할 때는 아래 구조를 그대로 따른다. 시장(또는 기업)이 여러 개면 개별 분석 파일을 하나씩 만들고, 비교가 필요하면 비교 전용 파일을 별도로 만든다.

### 4-1. 개별 시장/기업 분석 파일 (`value-chain-analysis-{slug}.md`)

1. **분석 대상 정의** — 분석 대상이 무엇인지 1~2문장으로 명시하고, 관련된 다른 분석 문서(예: 같은 시장의 Five Forces 분석)가 있으면 링크로 연결한다.
2. **분석 우선순위** — 9개 활동 중 원가 비중 또는 차별화가 큰 2~3개를 선정하고, *왜 그 활동을 골랐는지* 근거를 1~2문장으로 밝힌다. 나머지 활동은 이 단계에서 심층분석 대상이 아님을 명시한다. 이 단계에 **9개 활동 다이어그램(mermaid)에서 심층분석 대상만 색으로 강조**한 그림을 함께 넣어, 어디에 힘을 줬는지 한눈에 보이게 한다.
3. **심층 분석** — 우선순위로 선정한 활동마다 소제목을 두고, 설계 질문과 개선 질문에 대한 답을 질문표를 그대로 나열하지 않고 **서술형 분석**으로 작성한다. 각 활동 항목 끝에는 "개선 관점" 한 줄을 덧붙인다.
4. **나머지 활동 요약** — 심층분석 대상이 아닌 활동을 활동당 1~2줄 bullet로 짧게 짚는다. 완전히 생략하지 않는다(어떤 활동을 왜 얕게 다뤘는지 확인 가능해야 함).
5. **종합** — 개별 활동을 단순히 요약하는 것이 아니라, **활동 간 상호작용에서 드러나는 통찰**(예: 병목이 가치사슬의 어느 지점에 있는지, 하나의 자원이 여러 활동에 리스크와 기회를 동시에 만드는지)을 1문단으로 제시한다. 이 섹션이 분석 결과의 핵심 산출물이다.

### 4-2. 비교 분석 파일 (`value-chain-comparison-{a}-vs-{b}.md`, 시장이 2개 이상일 때)

1. **각 산업이 집중분석한 활동** 표 — 대상별 심층분석 활동과 선정 근거를 나란히 정리.
2. **활동별 비교** 표 — 9개 활동 전체를 행으로 두고 대상별 내용을 나란히 비교(심층분석 여부와 무관하게 9개 모두 포함).
3. **핵심 차이** — 병목의 위치 등 구조적 차이를 설명한다. 가능하면 가치사슬 순서(투입→운영→산출→마케팅→서비스)를 가로축으로 두고 **병목 위치를 표시하는 다이어그램**을 함께 넣는다.
4. **공통점** — 대상들이 공유하는 리스크·의존 구조를 짚고, 가능하면 관련 Five Forces 분석 결론과 연결한다.
5. **전략적 시사점** — 대상별로 우선 투입해야 할 자원/활동이 무엇인지 실행 가능한 결론으로 마무리한다.

### 4-3. 공통 규칙

- 방법론 문서(`value-chain-methodology.md`) 자체는 분석 결과와 분리해 항상 그대로 보존한다 — 실제 분석은 별도 파일에 쓴다.
- 질문표의 질문을 그대로 복사해 나열하지 않는다. 질문에 대한 **답**을 쓴다.
- 파일 간 상호 링크(Markdown 링크)를 적극 활용해, 개별 분석 → 비교 분석 → 관련 Five Forces 분석이 서로 참조되게 한다.
