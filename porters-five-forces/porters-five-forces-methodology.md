...
### 🖼️ 한눈에 보기 — 다섯 가지 힘이 산업을 포위하는 그림

```mermaid
graph TB
    SUP["🔧 공급자의 힘<br/>Supplier Power"]
    NEW["🚪 신규 진입자의 위협<br/>Threat of New Entrants"]
    IND(("🏭 산업 내 경쟁 강도<br/>Rivalry Among Competitors<br/><br/>= 산업 수익성"))
    SUB["🔄 대체재의 위협<br/>Threat of Substitutes"]
    BUY["💰 구매자의 힘<br/>Buyer Power"]
    ...
    classDef center fill:#f97316,stroke:#c2410c,color:#fff,stroke-width:2px;
    classDef force fill:#e0f2fe,stroke:#0369a1,color:#0c4a6e;
    class IND center;
    class SUP,NEW,SUB,BUY force;
```

> 네 방향에서 압박이 들어올수록(화살표가 굵어질수록) 산업 내부의 경쟁 강도가 세지고, 결과적으로 산업 전체의 평균 수익률은 낮아짐.
...
| 아이콘 | 힘 | 한 줄 정의 |
| :---: | --- | --- |
| 🔧 | 공급자의 힘 | 원재료·부품 공급자가 가격·조건을 좌우할 수 있는가 |
| 💰 | 구매자의 힘 | 고객이 가격·품질을 깎아내릴 수 있는가 |
| 🚪 | 신규 진입자의 위협 | 진입장벽이 낮아 새 경쟁자가 쉽게 들어올 수 있는가 |
| 🔄 | 대체재의 위협 | 다른 방식으로 같은 욕구를 채우는 대안이 있는가 |
| ⚔️ | 기존 경쟁 강도 | 이미 있는 경쟁사끼리 얼마나 치열하게 싸우는가 |
...
graph TD
    ...
    classDef forceNode fill:#e0f2fe,stroke:#0369a1,color:#0c4a6e;
    classDef resultNode fill:#fef3c7,stroke:#b45309,color:#78350f;
    classDef actionNode fill:#dcfce7,stroke:#15803d,color:#14532d;
    class B,C,D,E,F forceNode;
    class A resultNode;
    class G actionNode;
```
...
시장이 여러 개면 **개별 분석 파일을 하나씩** 만들고, 비교는 **별도 비교 파일**로 분리한다 — 하나의 파일에 모두 합치지 않는다.

### 4-1. 개별 시장 분석 파일 (`porters-five-forces-analysis-{slug}.md`)

1. **분석 단위 설정** — "우리"가 누구인지(분석 주체·수익 모델·지역·시점·참고 사례)를 표로 못 박는다. 구매자가 여러 종류(예: 광고주 vs 사용자, 소비자 vs 정부)면 반드시 분리해서 분석한다는 점을 경고 문구로 남긴다.
2. **공급자의 힘** — 공급자를 층(layer)으로 쪼갠 표로 제시하고, 층마다 힘의 세기와 근거를 적는다.
3. **구매자의 힘** — 구매자 유형별로 소제목을 나눠 각각의 세기를 판정한다.
4. **신규 진입자의 위협** — 방법론의 3요소(규모의 경제/기술·특허/채널·브랜드)를 표로 점검한 뒤, 표에서 드러나지 않는 이 산업 특유의 진입 경로(예: 인접 대기업의 기능 복제, 생태계 전체가 필요해 합작으로만 진입 가능 등)를 별도로 짚는다.
5. **대체재의 위협** — 고객의 욕구를 갈래로 분해한 표로 대체재를 나열하고, 그중 가장 치명적인 것을 짚는다.
6. **산업 내 경쟁 강도** — 경쟁 수단이 가격인지 비용 상승형(콘텐츠·기능 투자)인지를 구분해서 설명한다.
7. **종합** — mermaid 다이어그램(5개 힘 → 산업 매력도 → 전략 방향)과 함께, 산업 매력도를 "낮음/중간/높음" + 한 줄 성격 규정(예: 과밀형, 초기 고정형)으로 제시한다.
8. **전략 시사점** — 저비용/차별화/집중화 3가지 본원적 전략을 표로 평가(권장/비권장/조건부)하고, 필요하면 구조 전환 레버(수익모델 자체를 바꾸는 방안)를 보조 항목으로 추가한다.

### 4-2. 비교 파일 (`porters-five-forces-comparison-{a}-vs-{b}.md`)

1. **힘의 세기 비교 표** — 5개 힘 + 산업 매력도를 나란히.
2. **구조적 차이** — 병목·생애주기 위치 등 근본 차이를 mermaid로 시각화.
3. **공통점** — 공유하는 리스크·의존 구조.
4. **전략적 시사점 비교 표** — 전략 방향을 대상별로 나란히.

### 4-3. 공통 규칙

- 방법론 문서 자체는 분석 결과와 분리해 그대로 보존한다.
- 개별 분석 파일 상단에 서로(분석①↔분석②↔비교 파일)를 링크로 연결한다.
