...
> 비교 대상: [분석 ③ — 개인 맞춤형 자산관리](./porters-five-forces-analysis-personal-asset-management.md) · [비교 분석](./porters-five-forces-comparison-asset-management-vs-food-delivery.md)
...
> 📌 라이더 공급자는 [분석①(위치기반 SNS)](./porters-five-forces-analysis-location-sns.md)의 크리에이터와 구조가 닮았다 — 개별로는 약하지만 멀티호밍 때문에 집단적으로 플랫폼 정책에 큰 영향력을 행사한다.
...
**전국 단위 정면 진입은 막혀 있지만, "1인 가구 근거리 소용량"이라는 니치는 예외다.** 대형 플랫폼이 규모의 경제를 위해 최적화한 것은 넓은 배달 반경과 다양한 메뉴 구성이지, 좁은 반경·소용량 단건 배달이 아니다 — [분석①](./porters-five-forces-analysis-location-sns.md)의 "로컬 네트워크 효과가 방어자에게 불리하다"는 논리가 여기서도 유효하다.
